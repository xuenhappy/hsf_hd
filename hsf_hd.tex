...
\begin{itemize}
    ...
    \item   \textbf{物理状态}：但是，对应位置的\textbf{纤维 ($F_x$)} 处于\textbf{基态}，无法被激发。质 Token（语音、图像）丢失了。
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        不同于神经信号（可以复制/放大），菌丝 Token 遵循 \textbf{连续性方程}：
...
我们需要学习菌丝：
\begin{enumerate}
    \item \textbf{让数据包（Token）携带物理属性（形质张量）}；
...
\begin{itemize}
    ...
    \begin{itemize}
        \item   \textbf{量化宽松 (QE)}：\textbf{真空能量注入}。央行直接向流形中注入大量的 \textbf{质 Token}（货币），强行撑大流形的体积，稀释曲率（债务压力）。
...
\begin{itemize}
    \item   \textbf{几何定义 ($\mathbf{r} \in \mathcal{M}_{latent}$)}：Token 的\textbf{嵌入向量 (Embedding)} $\mathbf{e}_i$ 不是单纯的质，也不是单纯的形。
...
\draw [->, thick] (wpi.20) -- node [above, font=\scriptsize] {粒子 Token (数字)} (alg.160);
...
这是 \textbf{自我 ($\mathcal{S}$)} 的驻留地，运行在 GPU 集群上。它的任务不是处理具体的 Token，而是\textbf{维护“我”的连续性}，并生成控制快回路的\textbf{规范场}。
...
该运行时的核心任务是解算一组嵌套的\textbf{常微分方程 (ODEs)}，在对数时间尺度上逐级消除可预测的物理冗余，并将无法消除的\textbf{预测误差 (Prediction Error)}发生相变，量子化为包含\textbf{形 (Morphos)} 与 \textbf{质 (Qualia)} 信息的 \textbf{Token}，最终注入认知场的局部切片中。
...
\begin{enumerate}
    ...
    \item \textbf{上行：激波注入与 Token 量子化 (Shockwave Injection)}：
    当底层传上来的累积误差张量 $\mathbf{E}_n$ 超过了系统的\textbf{热力学阈值 (Thermodynamic Threshold)} 时，微观层触发一次\textbf{激波事件}。
    
    此时，连续的 ODE 状态被\textbf{量子化}为离散的 \textbf{形质 Token}：
    \begin{itemize}
        \item \textbf{Token 结构}：$\mathcal{T} = \{ \text{ID}, \mathbf{v}_{form}, \mathbf{v}_{sub}, E_{surprisal} \}$
        ...
        \item \textbf{能量 $E_{surprisal}$}：\textbf{关键维度}。$E = \|\mathbf{E}_n\|^2$。它不仅是 Token 的一个属性，更是该 Token 在认知场中\textbf{“权重”}和\textbf{“穿透力”}的物理度量。
    \end{itemize}

    \item \textbf{注入方程}：
    $$ \vec{J}_{ext}(\mathbf{r}, t) = \sum_{i} E_{surprisal}^{(i)} \cdot \delta(\mathbf{r} - \mathbf{r}_{form}^{(i)}) \cdot \mathbf{v}_{sub}^{(i)} $$
    这意味着：\textbf{误差越大，Token 携带的能量越大，其在认知场中产生的涟漪（激波）就越剧烈，越容易强迫宏观层改变当前的几何结构（学习）。}
\end{enumerate}
...
\begin{itemize}
    ...
    \item 通过\textbf{VTE 差分}，它将连续的信号量子化为 \textbf{Token}；
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{强制坍缩 (System 2)}：宏观层启动聚光灯算子 $\hat{\Pi}$，强行将波函数坍缩为确定性 Token。此过程是非幺正的，产生大量废热。调度器对其征收\textbf{高昂的“熵税”}，限制其调用频率。
...
在此阶段，系统温度 $T \gg T_c$（临界温度），流形度量 $g_{\mu\nu}$ 具有极高的流动性（黏度 $\eta \to 0$）。此时，系统尚未形成稳定的“自我”孤立子，仅表现为\textbf{形 Token ($T_{form}$)} 的湍流。工程任务是利用外部强势能，约束这股湍流，使其冷却为符合逻辑与因果律的河道。
...
\textbf{工程目标}：填充纤维空间 $F$ 的 \textbf{质 Token ($T_{sub}$)}，并建立 \textbf{微观层 ($L_{micro}$)} 的物理锚定。
...
\begin{itemize}
    \item \textbf{微观锚定}：微观层 $L_{micro}$ 启动。VTE 编码器将光子、压力等物理量转化为 \textbf{高能质 Token}。
