...
\section{形的量化：Epiplexity 作为结构的比特度量}

在确立了“形 (Morphos)”作为底流形的拓扑骨架之后，我们必须回答一个工程问题：如何度量“形”的复杂度？传统的香农熵无法区分“随机的混沌”与“复杂的结构”。为此，我们引入\textbf{Epiplexity (认知复杂度)} 作为 \textbf{形 Token ($V_S$)} 的物理量纲。

\begin{definition}[Epiplexity ($\mathrm{S}_T$): 形的度量]
    对于一个计算受限的观察者（时间界限为 $T$），对象 $X$ 的 \textbf{Epiplexity $\mathrm{S}_T(X)$} 定义为能够在该时间界限内生成 $X$ 的概率分布的最短程序长度。
    $$ \mathrm{S}_T(X) := |\mathrm{P}^\star|, \quad \text{where } \mathrm{P}^\star = \argmin_{\mathrm{P} \in \mathcal{P}_T} \left\{ |\mathrm{P}| + \mathbb{E}[-\log P(X)] \right\} $$
\end{definition}

\begin{itemize}
    \item \textbf{物理诠释}：
    \begin{itemize}
        \item $\mathrm{S}_T$ (Epiplexity) 对应于 \textbf{底流形 $\mathcal{M}$ 的几何复杂度}。它是智能体为了理解数据而在内部构建的\textbf{逻辑模型 (Model)} 的大小。
        \item $\mathrm{H}_T$ (Time-bounded Entropy) 对应于 \textbf{纤维空间 $F$ 的剩余热噪}。它是模型无法解释、只能作为\textbf{随机质料 (Noise)} 存储的部分。
    \end{itemize}
    \item \textbf{智能的偏好}：智能的演化方向，即是在固定总信息量的情况下，最大化 \textbf{形 ($\mathrm{S}_T$)} 的占比，最小化 \textbf{质 ($\mathrm{H}_T$)} 的残留。即：\textbf{将不可预测的命运，转化为可计算的几何结构。}
\end{itemize}
...
\begin{bquote}
    ...
    它是 Epiplexity 的\textbf{几何载体}，但仅仅提供了\textbf{无方向的路程}。要产生“难易之分”和“顺逆之别”，我们需要引入打破对称性的力量——这便是下一章的主角：\textbf{价值规范场}。
\end{bquote}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{动力学效应}：它让黎曼流形产生了 \textbf{倾斜}。思维流 $\Psi$ 在此作用下会自动流向“价值势能”更低的区域（趋利），并避开势能高的区域（避害）。Epiplexity 的不对称性在此体现为 \textbf{顺势做功} 与 \textbf{逆势做功} 的差异。
\end{itemize}
...
\begin{itemize}
    ...
    $$ \mathcal{F}_{\mu\nu} = \partial_\mu \mathcal{A}_\nu - \partial_\nu \mathcal{A}_\mu $$
    它不直接做功（不改变 Epiplexity 的标量大小），但改变思维流的方向与色彩。
    \item \textbf{语义映射}：
    \begin{itemize}
...
这个公式完美地统一了客观事实与主观价值，解释了 Epiplexity 的非对称性：
...
\begin{introduction}
    ...
    \item 纵向分量：Epiplexity 作为热力学难度 (电场项)
...
\begin{bquote}
    ...
    在 HSF-HD 的理论拼图中，\textbf{Epiplexity}（衡量计算的难易）与 \textbf{量子认知}（描述思维的非对易性）长期以来似乎描述着两种截然不同的现象。前者关乎\textbf{能量与做功}，后者关乎\textbf{状态与语境}。
...
\begin{itemize}
    \item \textbf{纵向力 ($\mathbf{F}_{\parallel}$)}：\textbf{改变速率}。它做功，改变系统的能量状态。这是 \textbf{Epiplexity} 的物理域。
    \item \textbf{横向力 ($\mathbf{F}_{\perp}$)}：\textbf{改变方向}。它不做功，但改变系统的相位与拓扑路径。这是 \textbf{量子认知} 的物理域。
\end{itemize}

\section{纵向分量：Epiplexity 作为热力学难度 (电场项)}
\label{sec:epiplexity_electric}

\textit{—— “山有多高？逆风有多大？”}

Epiplexity ($\mathrm{S}_T$) 的核心特征是\textbf{非对称性} ($\mathrm{S}_T(B|A) \neq \mathrm{S}_T(A|B)$)。在几何动力学中，这种非对称性只能来源于一个\textbf{保守力场}（如重力场或静电场）的存在。

\smalltitle{Epiplexity 的场论定义：标量势梯度}

我们将 Epiplexity 重新定义为潜语义流形上的\textbf{电势 (Electric Potential)} 或 \textbf{重力势} $\Phi(\mathbf{r})$。

\begin{definition}[Epiplexity 梯度]
...
这完美解释了 Epiplexity 论文中的“生成容易，逆推难”：
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item   \textbf{Epiplexity}：\textbf{极低}（接近于零或负值，表现为“灵感迸发”）。
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item   \textbf{Epiplexity}：\textbf{极高}（正比于势能差 $\Delta \Phi$）。
    \end{itemize}
\end{itemize}

\textbf{结论}：Epiplexity 测量的是\textbf{“逆熵做功”}的代价值。它是语义流形上的\textbf{“海拔高度差”}。
...
\begin{itemize}
    ...
    \item \textbf{几何相位}：思维流 $\Psi$ 回到原点时，会积累一个额外的相位因子（**阿哈罗诺夫-玻姆效应**）：
...
\begin{theorem}[认知洛伦兹力方程]
    智能体在潜语义流形上的动力学遵循：
    $$ \boxed{ \frac{d\mathbf{p}}{dt} = \underbrace{-\nabla V_{logic}}_{\text{黎曼引力 (惯性)}} + \underbrace{q \mathbf{E}}_{\text{Epiplexity (难度)}} + \underbrace{q \mathbf{v} \times \mathbf{B}}_{\text{Quantum Phase (语境)}} } $$
\end{theorem}
...
\begin{bquote}
    ...
    通过引入“电场”与“磁场”的几何对应，我们彻底厘清了 Epiplexity 与 量子认知 的关系：

    \textbf{Epiplexity 是语义流形的“坡度”，它决定了思考的代价（做功）；}
...
\begin{bquote}
    ...
    本章将利用\textbf{路径积分 (Path Integral)} 方法，揭示 Epiplexity 非对称性的物理根源。我们将证明，智能体在潜语义流形上的运动并非沿着单一的可逆轨迹，而是分裂为两条截然不同的路径：一条是顺应规范场的\textbf{“生成之路”}，另一条是逆抗规范场的\textbf{“推理之路”}。智能的奥秘，就隐藏在这两条路径围成的\textbf{磁滞回线 (Hysteresis Loop)} 之中。
\end{bquote}

\section{双路径模型：事实距离 vs. 思维距离}
\label{sec:dual_path_model}

在 HSF-HD 的“黎曼+规范场”架构下，我们必须严格区分两个维度的“距离”。
...
这是智能体实际执行计算过程（程序）时所消耗的\textbf{热力学作用量}。它对应于 Epiplexity。
...
Epiplexity 论文中提到的 $y|x$（生成）与 $x|y$（推断），在 HSF-HD 中不再是同一条路径的往返，而是相空间中的两条\textbf{拓扑异构}轨迹。
...
\begin{itemize}
    ...
    \item   \textbf{Epiplexity}：\textbf{极低}。程序的长度仅相当于“初始条件”的长度，后续演化由物理定律（模型权重）自动完成。
\end{itemize}
...
\begin{itemize}
    ...
    \item   \textbf{Epiplexity}：\textbf{极高}。程序不仅要包含路程，还要包含大量的\textbf{纠错码}、\textbf{搜索算法}和\textbf{验证步骤}，以对抗熵增发散的趋势。
\end{itemize}
...
本章通过路径积分的方法，彻底几何化了 Epiplexity 的不对称性。

\begin{theorem}[认知做功定理]
    智能体对知识的“理解程度”，不取决于它记住了多少（黎曼距离），而取决于它克服了多少规范场阻力（做功）。
    $$ \text{Understanding} \propto \oint_{\text{Loop}} \mathbf{F}_{cog} \cdot d\mathbf{r} = \Delta \text{Epiplexity} $$
\end{theorem}

\begin{itemize}
    \item   \textbf{顺流 (Generating)} 是\textbf{才华}，是能量的释放，是 $\mathrm{S}_T$ 的极小值。
    \item   \textbf{逆流 (Understanding)} 是\textbf{功力}，是能量的注入，是 $\mathrm{S}_T$ 的极大值。
\end{itemize}
