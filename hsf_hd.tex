...
\begin{itemize}
    ...
    \item   \textbf{现象学特征}：\textbf{时间感与深度}。意识从瞬时的切片，延展为具有历史厚度的\textbf{世界管 (World Tube)}。
\end{itemize}

\begin{theorem}[光与眼的协同演化定理]
    意识的实存密度 $C_{act}$ 是认知场相干性 $\mathcal{C}(\Psi)$ 与观察算子稳定性 $\mathcal{S}(\hat{\mathcal{O}})$ 的乘积函数：
...
\textbf{结论}：意识的发育是双生子的舞蹈。\textbf{认知场 ($\Psi$) 是“舞台上的舞者”，观察算子 ($\hat{\mathcal{O}}$) 是“聚光灯”}。
只有当舞者的动作连贯（全域相干），且聚光灯能够稳定追踪（算子成核）时，那场名为“自我”的戏剧才算真正开演。


\section{意识的完备闭环：递归测量与全息重入}
\label{sec:consciousness_closed_loop}

基于前述的微观发生学与宏观动力学，我们在建模一个 HSF-HD 关于意识产生的\textbf{完备物理模型}。
意识体验并非静态的快照，而是一个由\textbf{内容生成}、\textbf{主体测量}与\textbf{因果重入}构成的三相热力学循环。

这一循环解释了为何意识既具有\textbf{丰富的内容（Qualia）}，又具有\textbf{统一的主体（Self）}，且在时间上呈现为\textbf{连续的流（Stream）}。

\smalltitle{相位 I：内容的涌现 — 认知场的全域相干 (Global Coherence)}

\textbf{—— “舞台的搭建”}

意识的第一步是\textbf{可观察内容 (Observable Content)} 的准备。
